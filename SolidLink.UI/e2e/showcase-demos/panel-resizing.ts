...
export const demoPanelResizing = async (page: Page) => {
  ...
  const sidebarHandle = await page.$('[data-testid="resize-sidebar"]');
  ...
  const treeHandle = await page.$('[data-testid="resize-tree"]');
  ...
  const robotHandle = await page.$('[data-testid="resize-robot"]');
